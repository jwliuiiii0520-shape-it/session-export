...
有些 Codex Session 不只是一次临时问答。

它可能包含一段完整的问题分析、一次重要决策的推导过程、值得复用的提示词，或一个项目
从定位问题到落地修改的完整上下文。只把它留在历史列表里，之后往往很难快速找到，也不
方便完整回顾。

`session-export` 可以把重要的 Codex 对话导出为 Markdown、PDF，或直接保存到
Obsidian。导出的不是一段简略摘要，而是一份适合长期保存和重新阅读的完整会话记录：
保留用户消息、Codex 回复和工具调用摘要，默认排除内部提示词，并在顶部附上简短摘要和
实际保存路径。

## 为什么值得使用？

- **完整复盘**：保留重要 Session 的完整对话，之后可以重新查看分析过程，而不只是最终答案。
- **知识沉淀**：用 Markdown 建立可搜索的长期会话知识库，让高价值对话不再散落在历史列表里。
- **项目交接**：把关键讨论、决策背景和处理过程保存下来，方便自己或协作者快速理解上下文。
- **Obsidian 归档**：输入 `se-obsi`，直接把当前对话保存到个人知识库。
- **离线阅读与分享**：需要回顾或分享时，生成排版清晰的 PDF。
- **可选审计**：只有明确开启审计模式时，才附带工具调用详情。
